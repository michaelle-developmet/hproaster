...
const submitFormRoasting = async () => {
  try {
    ...
    const { data } = await axios.post('http://localhost:5011/api/update-roasting', updatedFormData, {
      withCredentials: true,
    });
...
const submitFormFinishRoasting = async () => {
  try {
    ...
    // Отправляем данные на сервер
    const { data } = await axios.post('http://localhost:5011/api/finish_roasting', updatedFormData, {
      withCredentials: true,
    });
